...
def evaluateAllDocs():
    #    
    strFolderDocName = "ArSummarizer/evaluation/GeneratedDocuments/"
    if not os.path.exists(strFolderDocName):
      os.makedirs(strFolderDocName)

    docListNames = [f for f in os.listdir(strFolderDocName) if os.path.isfile(os.path.join(strFolderDocName, f))]
    docListNames = [f for f in listdir(strFolderDocName) if isfile(join(strFolderDocName, f))]
    #
    #
    iCount=0
    with open("ArSummarizer/evaluation/eval.csv",'w') as statsCSV:
        #
        #
